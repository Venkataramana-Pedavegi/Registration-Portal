...
const AIChatWidget = () => {
  ...
  useEffect(() => {
    sessionStorage.setItem('svec_chat_history', JSON.stringify(messages));
    if (isOpen) {
  ...
  const handleClearHistory = () => {
    ...
    setSessionId('');
    sessionStorage.removeItem('svec_chat_history');
  };
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {!isOpen && (
        <button
          ...
          className="p-4 bg-gradient-to-r from-indigo-650 to-purple-650 text-white rounded-full shadow-2xl hover:scale-105 transition-all flex items-center gap-2 group border border-white/20"
        >
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-80 sm:w-96 h-[500px] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-4 text-white flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-purple-200 font-medium">Sri Vasavi Intelligent Bot</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-1 hover:bg-white/20 rounded-lg text-xs font-bold"
              >
                Clear
              </button>
              <button
          ...
          <div className="flex-grow p-4 overflow-y-auto space-y-3 bg-gray-50/50 text-xs">
            {messages.map((m, idx) => (
              ...
              >
                {m.sender === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center flex-shrink-0 mt-0.5 font-bold">
                    AI
                  </div>
                )}
                <div
          ...
          <div className="px-3 py-1.5 bg-gray-100/70 border-t border-gray-100 flex gap-1.5 overflow-x-auto text-[10px]">
            <button
              onClick={() => handleQuickPrompt('Which events are available?')}
              className="px-2 py-1 bg-white hover:bg-indigo-50 text-gray-600 rounded-md border text-nowrap"
            >
              ⭐ Available Events
            </button>
            <button
              onClick={() => handleQuickPrompt('Show my registrations.')}
              className="px-2 py-1 bg-white hover:bg-indigo-50 text-gray-600 rounded-md border text-nowrap"
            >
              🎓 My Registrations
            </button>
            <button
              onClick={() => handleQuickPrompt('How do I become a volunteer?')}
              className="px-2 py-1 bg-white hover:bg-indigo-50 text-gray-600 rounded-md border text-nowrap"
            >
